...
from yoyopod_cli.pi.support.music_backend.models import MusicConfig, Track
...
class MpvBackend:
        ...
        with self._start_lock:
            ...
            try:
                for attempt in range(1, self._STARTUP_SPAWN_ATTEMPTS + 1):
                    ...
                    connect_deadline = time.monotonic() + self._STARTUP_CONNECT_TIMEOUT_SECONDS
                    connected = False
                    while time.monotonic() < connect_deadline:
                        if self._can_attempt_ipc_connect() and self._connect_ipc_quietly():
                            connected = True
                            break
    ...
    def get_time_position(self) -> int:
        ...
        if is_stale:
            ...
            if should_log_stale:
                logger.warning(
                    "mpv time-pos cache went stale during playback; " "returning 0 progress"
                )
            return 0
