...
| `yoyopod remote mode activate` | Activate dev or prod, stopping conflicting runtime and legacy services first. |
...
| `yoyopod remote restart` | Restart the YoYoPod dev runtime service on the Pi. |
...
| `yoyopod remote sync` | Update the dev lane checkout from Git and restart the dev runtime service. |
...
| `yoyopod pi validate deploy` | Validate deploy-readiness for the current target checkout without launching the runtime. |
...
| `yoyopod build ensure-native` | Build missing or stale native artifacts for local development. |
| `yoyopod build lvgl` | Build the pinned upstream LVGL library for the current platform. |
| `yoyopod build rust-runtime` | Build the Rust runtime binary. |
| `yoyopod build rust-ui-host` | Build the Rust UI host binary. |
| `yoyopod build simulation` | Build the LVGL native library used to simulate the device UI host. |
...
| Command | What it does |
|---|---|
| `yoyopod health live` | Report the running version. Exit 0 = release detected AND service active. |
| `yoyopod health preflight` | Offline structural check of a release slot. Exit 0 = OK. |
