...
use yoyopod_network::runtime::{NetworkRuntime, RecoveryPolicy};
use yoyopod_network::snapshot::NetworkLifecycleState;
use yoyopod_network::worker::{
...
fn worker_keeps_runtime_snapshot_across_query_gps_and_health_commands() {
    ...
    assert_eq!(
        ...
        yoyopod_network::protocol::EnvelopeKind::Result
    );
...
fn worker_emits_network_error_for_command_triggered_health_fault() {
    let modem = FakeModemController::new();
    modem.set_ppp_health_results([yoyopod_network::modem::PppHealth::ProcessExited]);
    let runtime = NetworkRuntime::new("config", enabled_config(), modem);
    let input = encode_commands(&[
        command("network.health", "health-1", json!({})),
    ...
    let error = envelopes
        .iter()
        .find(|envelope| envelope.kind == yoyopod_network::protocol::EnvelopeKind::Error)
...
fn worker_reset_modem_returns_recovered_snapshot_and_shutdown_stops_runtime() {
    ...
    assert_eq!(
        envelopes
            .iter()
            .find(|envelope| {
                envelope.kind == yoyopod_network::protocol::EnvelopeKind::Result
...
fn worker_emits_network_error_for_gps_query_failure() {
    ...
    let error = envelopes
        .iter()
        .find(|envelope| {
            envelope.kind == yoyopod_network::protocol::EnvelopeKind::Error
